...
export default function LoteriaDashboard() {
  ...
  const { data: dashData, isLoading } = useQuery({
    queryKey: ['loteria-dashboard', eventId],
    queryFn: () => getLoteriaDashboard(eventId!),
    enabled: !!eventId,
  ...
  // Preparar datos para gráfica de area (ventas por día)
  ...
  // Datos para bar chart de agencias
  const agenciasBarData = useMemo(() => {
    if (!dashboard?.agencias) return [];
    return dashboard.agencias.map(a => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Dashboard Loteria</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Estadisticas de ventas por agencias de loteria
          </p>
        </div>
        <Select
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <StatCard
              ...
              value={formatNumber(dashboard.resumen.total_cartones)}
              color="blue"
            />
            <StatCard
              icon={ShoppingCart}
              label="Vendidos"
              value={formatNumber(dashboard.resumen.cartones_vendidos)}
              color="emerald"
            />
            <StatCard
              icon={CreditCard}
              label="Disponibles"
              value={formatNumber(dashboard.resumen.cartones_disponibles)}
              color="amber"
            />
            <StatCard
              icon={Package}
              label="Total Cajas"
              value={formatNumber(dashboard.resumen.total_cajas)}
              color="violet"
            />
            <StatCard
              icon={BarChart3}
              label="Total Lotes"
              value={formatNumber(dashboard.resumen.total_lotes)}
              color="cyan"
            />
            <StatCard
              icon={TrendingUp}
              label="% Vendido"
              value={`${dashboard.resumen.porcentaje_vendido}%`}
              color="rose"
            />
          </div>

          {/* Charts Row 1: Ventas por día + Pie de vendidos/disponibles */}
          ...
          {/* Charts Row 2: Agencias bar chart + Ranking */}
          {dashboard.agencias.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card className="lg:col-span-2">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Building2 className="h-5 w-5 text-violet-500" />
                    Cartones por Agencia
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-left">
                        <th className="py-3 px-2 font-semibold">Agencia</th>
                    ...
                    <tbody>
                      ...
                      <tr className="bg-muted/50 font-bold">
                        ...
                        <td className="py-3 px-2 text-center">{formatNumber(dashboard.agencias.reduce((s, a) => s + a.total_cajas, 0))}</td>
                        <td className="py-3 px-2 text-center">{formatNumber(dashboard.agencias.reduce((s, a) => s + a.total_lotes, 0))}</td>
                        <td className="py-3 px-2 text-center">{formatNumber(dashboard.agencias.reduce((s, a) => s + a.total_cartones, 0))}</td>
                        <td className="py-3 px-2 text-center text-emerald-600">{formatNumber(dashboard.agencias.reduce((s, a) => s + a.cartones_vendidos, 0))}</td>
                        <td className="py-3 px-2 text-center">{formatNumber(dashboard.agencias.reduce((s, a) => s + a.cartones_disponibles, 0))}</td>
                        <td className="py-3 px-2 text-center">
          ...
          {dashboard.agencias.length === 0 && (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-lg font-semibold mb-1">Sin agencias configuradas</h3>
                <p className="text-sm text-muted-foreground">
                  Marca almacenes como "Agencia de Loteria" en la seccion de Inventario para que aparezcan aqui.
                </p>
              </CardContent>
            </Card>
          )}
